...
import 'package:pelgrim/pages/user/images_upload/all_images/all_images_page.dart';
      ...
      if (!_selectedImages.any((selectedImage) => selectedImage.name == image.name)) {
        try {
          ...
          ScaffoldMessenger.of(context)
              .showSnackBar(const SnackBar(content: Center(child: Text('Wystąpił problem'))));
        }
      }
    }
    setState(() {});
                    ...
                    InkWell(
                      onTap: () async => {
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => AllImagesPage(settings: widget.settings)))
                      },
                      child: Container(
                        height: 50,
