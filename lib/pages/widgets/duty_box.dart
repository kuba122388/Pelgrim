...
class _DutyBoxState extends State<DutyBox> {
  ...
  bool get isUserSignedUp {
    return duty.volunteers.any((user) => user.email == widget.currentUser.email);
  }
  ...
  Future<void> _toggleSignup() async {
    ...
    setState(() {
      if (isSignedUp) {
        duty.volunteers.removeWhere((user) => user.email == widget.currentUser.email);
      } else {
            ...
            if (duty.volunteers.isNotEmpty)
              Column(
                ...
                children: [
                  ...
                  ...duty.volunteers.map((u) => Text('${u.firstName} ${u.lastName}')),
                ],
              ),
          ],
        ),
